...
// FormatRupiah mengubah float32 jadi "Rp 1.234.567"
...
func ParseRupiahLikeToFloat(s string) (float32, error) {
	// ambil hanya digit
	var b strings.Builder
	b.Grow(len(s))
	for _, r := range s {
...
func ReadRupiahInteractive(prompt string) (float32, error) {
	fd := int(os.Stdin.Fd())
